...
function left_menu_click(url,id,e,title){
	...
	if($('#'+id).length==0){
		...
		var add_html_div='<div class="ui bottom attached active tab content_div" data-tab="'+id+'" id="'+id+'_div" style="height:100%">正在加载 (>^ω^<)喵</div>';
		$($.parseHTML(add_html, document, true)).appendTo($('#content .menu'));
		$($.parseHTML(add_html_div, document, true)).appendTo($('#content #content_divs'));
		var iframe=
			'<iframe frameborder="0" src="' + url 
			+ '" width="100%" height="100%" id="iframe-container-' + id
			+ '" name="iframe-container-' 
			+ id
			+ '" class="iframe-container"  scrolling="auto"></iframe>';
